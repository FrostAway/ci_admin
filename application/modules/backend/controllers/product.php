...
class Product extends MX_Controller {
    ...
    public function index() {
        ...
        $products = $this->product_model->view();
        $this->load->model('attr_relation_model');
        $data = array();
        foreach ($products as $product) {
            $image = $this->attr_relation_model->get_value($product['id'], 1);
            if ($image == null) {
    ...
    public function update($id) {
        ...
        $pass['product'] = $this->product_model->get_product($id);
    ...
    public function addnew() {
        ...
        if ($this->input->post('btn-addnew')) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $data = $this->input->post('product');
                $data['slug'] = convert_tv($data['name']);
                $p_id = $this->product_model->insert($data);
